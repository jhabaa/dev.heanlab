...
async function load(){
    ...
    prototype.value = prototypeStore.data.find(p => 
        p.Name == currentPrototypeName
    ) as Prototype
    await nextTick();
...
<template>
    <main>
        <TitleItem :title="prototype?.Title" :subtitle="prototype?.keywords " :position="titlePosition.Top"></TitleItem>
    </main>
</template>
