...
<template>
  <div class="wrapper-title" :class="position">
    ...
    <div :class="position" class="block">
      <h1>
        {{
          projectsStore.getSelectedProject
            ? projectsStore.getSelectedProject.Name
            : title
        }}
      </h1>
      <h3>
        {{
          projectsStore.getSelectedProject
            ? projectsStore.getSelectedProject.Title
            : subtitle
        }}
      </h3>
      <div class="content">
        <div class="github" v-if="githubLink">
          ...
          <github-button
            :href="'https://github.com/jhabaa/' + githubLink"
            data-color-scheme="no-preference: light_high_contrast; light: dark_dimmed; dark: dark;"
            data-icon="octicon-eye"
            data-size="large"
            data-show-count="true"
            aria-label="Watch jhabaa/jhabaa on GitHub"
            >Watch</github-button
          >
        </div>
        <div class="github" v-else>
          <github-button
            href="https://github.com/jhabaa"
            data-color-scheme="no-preference: light; light: light; dark: dark;"
            data-size="large"
            data-show-count="true"
            aria-label="Follow @jhabaa on GitHub"
            >Visit @jhabaa</github-button
          >
        </div>
      </div>
    </div>
...
.wrapper .dark {
  ...
  background: radial-gradient(
    circle,
    rgba(15, 15, 15, 0) 0%,
    rgba(15, 15, 15, 0.6551412801448704) 10%,
    rgba(0, 0, 0, 1) 100%
  );
